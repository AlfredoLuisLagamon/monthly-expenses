import { getSheetsClient, validateWorkbook, readSheet, appendRows, updateRow, deleteRow, ensureMonth, ensureMonthFromData, } from './sheets.js';
import { SHEET_NAMES, MASTER_HEADERS, MONTHLY_HEADERS, STATUS_PAID, STATUS_UNPAID, } from './constants.js';
...
function getSheetId(req) {
    const id = req.query.sheetId ?? req.body?.sheetId;
    return id?.trim() || null;
}
...
export async function getData(req, res) {
    ...
    try {
        ...
        const header = monthlyData?.[0] ?? ['MonthYear', 'ExpenseId', 'Status', 'Amount'];
        ...
        const monthly = monthlyAll.filter((r) => r.MonthYear === month);
...
export async function postEnsureMonth(req, res) {
    const sheets = getSheets(req);
    const sheetId = getSheetId(req);
    const month = getMonth(req);
...
export async function patchMonthlyStatus(req, res) {
    const sheets = getSheets(req);
    const sheetId = getSheetId(req);
...
async function updateMonthlyStatusCell(sheets, spreadsheetId, rowIndex, status) {
...
export async function getValidate(req, res) {
    const sheets = getSheets(req);
    const sheetId = getSheetId(req);
    if (!sheets || !sheetId) {
...
export async function postMaster(req, res) {
    const sheets = getSheets(req);
    const sheetId = getSheetId(req);
...
export async function putMaster(req, res) {
    const sheets = getSheets(req);
    const sheetId = getSheetId(req);
...
export async function postPayor(req, res) {
    const sheets = getSheets(req);
    const sheetId = getSheetId(req);
    const name = (req.body?.name ?? '').toString().trim();
    if (!sheets || !sheetId || !name) {
...
export async function deletePayor(req, res) {
    const sheets = getSheets(req);
    const sheetId = getSheetId(req);
    const rowIndex = Number(req.params.rowIndex);
    if (!sheets || !sheetId || !Number.isInteger(rowIndex) || rowIndex < 2) {
